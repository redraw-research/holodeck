...
def _kill_recursive(proc_pid: int, only_terminate: bool = True, timeout_seconds: int = 5, children_from_previous_attempt = None):
    try:
    ...
    try:
        children = children_from_previous_attempt or process.children(recursive=True)
        if only_terminate and process:
            try:
                process.terminate()
        ...
        for proc in children:
            if only_terminate:
                ...
                try:
                    ...
                    pass
        if not only_terminate and process:
            try:
                process.kill()
            except psutil.NoSuchProcess:
                pass
...
class HolodeckEnvironment:
    ...
    Args:
        ...
        ticks_per_sec (:obj:`int`, optional):
            Number of frame ticks per simulated unreal second. Defaults to 30.
            Specify 0 or None for unlocked framerate like a normal video game.
    ...
    def __init__(
        ...
        self._reset_ptr = self._client.malloc("RESET", [1], bool)
            ...
            for sensor in agent["sensors"]:
                ...
                sensors.append(
                    SensorDefinition(
                        ...
                        existing=sensor_config["existing"]
                    )
                )
    ...
    def reset(self, load_new_agents: bool = True):
    ...
    def __linux_start_process__(
        ...
        command = [
            ...
            # "-LOG=HolodeckLog.txt",
                "-NOWRITE",  # disable logging (for very long training runs)
        ...
        if not show_viewport:
            ...
            if os.getenv("CUDA_VISIBLE_DEVICES"):
                extra_args.append(f"-GraphicsAdapter={os.getenv('CUDA_VISIBLE_DEVICES').split(',')[0]}")

                # https://www.reddit.com/r/linux_gaming/comments/uibl1g/how_to_force_a_game_to_use_gpu/
                environment["NODEVICE_SELECT"] = "1"
            command[-4:-4] = extra_args
        else:
            command[-4:-4] = [
                "-log",
            ]
        ...
        if self._ticks_per_sec:
            # --HolodeckUUID needs to be last argument
            command[-2:-2] = [
                "-BENCHMARK",  # Run game at fixed-step in order to process each frame without skipping any frames.
                "-FPS=" + str(self._ticks_per_sec),  # Set the frames per second for benchmarking.
                "-TicksPerSec=" + str(self._ticks_per_sec),  # (author1) not sure if this does anything
            ]
        ...
        try:
            loading_semaphore.acquire(10 * 30)
    ...
    def __on_exit__(self):
        ...
        if hasattr(self, "_world_process"):
            try:
                try:
                    children = psutil.Process(self._world_process.pid).children(recursive=True)
                except psutil.NoSuchProcess:
                    children = None

                _kill_recursive(self._world_process.pid)
            except psutil.TimeoutExpired:
                num_retries = 5
                for attempt in range(num_retries):
                    try:
                        print(f"Retrying Unreal Engine Process Kill: {attempt+1}/{num_retries}")
                        _kill_recursive(self._world_process.pid, only_terminate=False, children_from_previous_attempt=children)
                    except psutil.TimeoutExpired:
