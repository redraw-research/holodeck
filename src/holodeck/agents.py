...
class DuckiebotAgent(HolodeckAgent):
    """An ETH Zurich Duckiebot DB21.
    ...
    **Action Space:**
    ...
    # constants in DuckiebotAgent.h in holodeck-engine
    __DISCRETE_MIN = 0
    __DISCRETE_MAX = 4

    __MAX_ROTATION_SPEED = 1.0
    __MIN_ROTATION_SPEED = -__MAX_ROTATION_SPEED

    __MAX_FORWARD_SPEED = 1.0
    __MIN_FORWARD_SPEED = -__MAX_FORWARD_SPEED
    ...
    @property
    def control_schemes(self):
        cont_min = [self.__MIN_FORWARD_SPEED, self.__MIN_ROTATION_SPEED, -1, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf, -np.inf]
        cont_max = [self.__MAX_FORWARD_SPEED, self.__MAX_ROTATION_SPEED, 1, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf]
        return [
            (
                "0: Move forward\n1: Move backward\n2: Turn right\n3: Turn left",
                DiscreteActionSpace(
            ...
            (
                "[forward_movement, rotation]",
                ContinuousActionSpace([8], low=cont_min, high=cont_max),
            ),
        ]
    ...
    def __repr__(self):
        return "DuckiebotAgent " + self.name
